            ...
            try {
                ...
                const builtinReactWidgets = [
                    ...
                    },
                    {
                        key: 'tickets-widget',
                        name: 'Tickets Widget',
                        description: 'Manage and view tickets',
                        type: 'builtin-react',
                        path: null
                    },
                    {
    ...
    const handleOpenSearchResult = (result) => {
        ...
        const newTab = {
            ...
            color: result.type === 'contact' || result.type === 'contacts' ? 'blue' :
                   ...
                   result.type === 'sale' || result.type === 'sales' ? 'indigo' :
                   result.type === 'task' || result.type === 'tasks' ? 'teal' : 'blue'
        };
        ...
        } else if (result.type === 'task' || result.type === 'tasks') {
            // For tasks, use the task profile widget
            resultLayout = [{
                i: `task-profile-${result.id}`,
                x: 0,
                y: 0,
                w: 12,
                h: 8,
                widgetKey: 'task-profile-widget',
                widgetData: { taskId: result.id }
            }];
        } else {
            resultLayout = [{
